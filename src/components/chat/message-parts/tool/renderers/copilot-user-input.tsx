...
import { unwrapCopilotInput } from "./copilot-helpers";
...
function isUserInputInput(value: unknown): value is CopilotUserInputInput {
  if (!value || typeof value !== "object") return false;
...
function parseUserInput(rawInput: unknown): ParsedUserInput | null {
  const unwrapped = unwrapCopilotInput<unknown>(rawInput);
...
export const CopilotUserInputTool = memo(function CopilotUserInputTool({
  ...
  const answeredResponse =
    approvalRecord && typeof approvalRecord.response === "string"
      ? approvalRecord.response
      : "";
  const [currentStep, setCurrentStep] = useState(0);
  const [response, setResponse] = useState(answeredResponse);
  ...
  const summary = (
    <>
      ...
      />
      {isDone ? "Input provided" : "GitHub Copilot is requesting input"}
    </>
  );
  ...
  return (
    ...
    >
      {isWaiting ? (
        ...
        <div className="rounded-xl border border-border/40 bg-surface/20 px-3 py-2 text-[12px] text-foreground/70 whitespace-pre-wrap break-words">
          {answeredResponse}
        </div>
      ) : null}
    </ToolLayout>

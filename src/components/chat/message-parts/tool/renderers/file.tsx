...
function parseDiff(raw: string): {
  ...
  for (const rawLine of raw.split("\n")) {
    ...
    if (rawLine.startsWith("+")) {
      additions++;
      lines.push({ kind: "add", text: rawLine.slice(1), oldNum: null, newNum: newNum++ });
    } else if (rawLine.startsWith("-")) {
      deletions++;
      lines.push({ kind: "del", text: rawLine.slice(1), oldNum: oldNum++, newNum: null });
    } else {
      lines.push({ kind: "ctx", text: rawLine.slice(1), oldNum: oldNum++, newNum: newNum++ });
    }
  }
...
function DiffView({ diff, path }: { diff: string; path: string }) {
  ...
  const { additions, deletions, allLines, groups, wordDiffs } = useMemo(() => {
    ...
    return { additions: a, deletions: d, allLines: lines, groups: g, wordDiffs: wd };
  }, [diff]);
  ...
  const renderLine = (line: DiffLine, absIdx: number) => {
    ...
    return (
      ...
      >
        ...
        <span
          className={isCtx ? "text-foreground/40" : "text-foreground/80"}
        >
          {isDel && wd ? (
...
function buildSummary(
  ...
  if (isError) {
    return (
      <>
        Failed to {toolName === "create_file" ? "create" : toolName === "delete_file" ? "delete" : "edit"}{" "}
    ...
    if (toolName === "multiedit" && isMultiEditOutput(part.output)) {
      return (
        <>
          ...
          <span className="font-mono text-[12px]">{part.output.path}</span>
          <span className="ml-1.5 text-[11px] text-foreground/40">
            {part.output.editsApplied} edit{part.output.editsApplied === 1 ? "" : "s"}
          </span>
        </>
      );
    }
...
function buildBody(
  ...
  if (part.state === "output-denied") {
    return (
      <p className="text-[11px] text-muted">Execution denied.</p>
    );
  }

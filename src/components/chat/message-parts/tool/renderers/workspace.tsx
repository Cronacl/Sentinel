...
function buildSummary(toolName: string, part: RendererProps["part"]): ReactNode {
  if (toolName === "diff") {
    const output =
      "output" in part && isDiffOutput(part.output) ? part.output : null;
    const input = "input" in part && isDiffInput(part.input) ? part.input : null;
    ...
    return (
      <>
        Generating diff for <span className="font-mono text-[12px]">{shownPath}</span>
      </>
    );
  }
  ...
  if (toolName === "move_file") {
    ...
    const input = "input" in part && isMoveFileInput(part.input) ? part.input : null;
  ...
  if (toolName === "git") {
    const output = "output" in part && isGitOutput(part.output) ? part.output : null;
    const input = "input" in part ? (part.input as { action?: string } | undefined) : undefined;
  ...
  return (
    <>
      {part.state === "approval-requested" && rationale ? (
        <p className="mb-1.5 line-clamp-2 text-[11px] text-muted">{rationale}</p>
      ) : null}
      {showApprovalActions ? (
  ...
  switch (output.action) {
    case "status":
      return (
        <div className="space-y-3 font-mono text-[11px] text-foreground/70">
          ...
          <p>
            Clean: {output.isClean ? "yes" : "no"} · staged {output.staged.length} ·
            unstaged {output.unstaged.length} · untracked {output.untracked.length}
          </p>
    ...
    case "diff":
      return output.diff ? (
        ...
        <p className="font-mono text-[11px] text-foreground/70">No diff output.</p>
      );
    case "log":
      return (
        <div className="space-y-2 font-mono text-[11px] text-foreground/70">
          {output.commits.map((commit) => (
            <div key={commit.hash}>
              <p>{commit.hash.slice(0, 12)} · {commit.subject}</p>
              <p className="text-foreground/40">{commit.author} · {commit.date}</p>
            </div>
          ))}
        </div>
      );
    ...
    case "branch_create":
      return <p className="font-mono text-[11px] text-foreground/70">{output.name}</p>;
    case "checkout":
      return <p className="font-mono text-[11px] text-foreground/70">{output.branch}</p>;
    case "add":
      return (
        <div className="space-y-1 font-mono text-[11px] text-foreground/70">
...
function renderBody(toolName: string, part: RendererProps["part"], errorText?: string) {
  ...
  if ("output" in part && part.output !== undefined) {
    if (toolName === "diff" && isDiffOutput(part.output)) {
      return part.output.diff ? (
        ...
        <p className="font-mono text-[11px] text-foreground/70">No changes detected.</p>
      );
    }
      ...
      return (
        <div className="flex flex-col gap-2">
          {part.output.files.map((file) => (
            <DiffView key={`${file.path}:${file.operation}`} diff={file.diff} path={file.path} />
          ))}
        </div>
      );
    }
    ...
    if (toolName === "diagnostics" && isDiagnosticsOutput(part.output)) {
      return (
        <div className="space-y-3">
          <p className="text-[11px] text-foreground/60">{part.output.summary}</p>
          <div className="space-y-2 font-mono text-[11px] text-foreground/70">

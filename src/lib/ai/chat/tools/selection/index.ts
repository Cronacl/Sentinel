...
function hasForcedToolTag(
  promptContext: ThreadPromptContext,
  tag: "browser" | "computer",
) {
  return promptContext.toolTags?.includes(tag) ?? false;
}
...
export function selectAlwaysOnChatTools({
  ...
  if (
    hasForcedToolTag(promptContext, "browser") ||
    userRequestNeedsBrowserTools(promptContext)
  ) {
    selectBrowserTools(activeTools, availableToolNames);
  }

  if (
    hasForcedToolTag(promptContext, "computer") ||
    userRequestNeedsComputerTools(promptContext)
  ) {
